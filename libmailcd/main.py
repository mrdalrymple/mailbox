...
def workflow_inbox_run(workspace, pipeline_inbox):
    env_vars = []
...
def cli_pull(workspace):
    ...
    if pipeline_inbox:
        workflow_inbox_run(workspace, pipeline_inbox)


@cli.command("clean")
@click.option("--workspace", default=".")
def cli_clean(workspace):
    arg_workspace = Path(workspace).resolve()
    pipeline_filepath = Path(arg_workspace, INSOURCE_PIPELINE_FILENAME).resolve()
...
def cli_build(project_dir):
    ...
    try:
        ...
        if pipeline_inbox:
            print(f"========== INBOX ==========")
            env_vars = workflow_inbox_run(arg_project_dir, pipeline_inbox)
            print(f"===========================")
        #######################################
